...
BOOL check_for_card(const char* RENDERER, const char* bad_card)
{
	if (!strnicmp(RENDERER, bad_card, strlen(bad_card)))
	{
		std::string buffer = llformat(
			"Your video card appears to be a %s, which The Universe Viewer does not support.\n"
			"\n"
			"Universe Viewer requires a video card with 32 Mb of memory or more, as well as\n"
			...
			"Universe Viewer Support group in world, or visit www.virtual-planets.org\n"
			"for support.\n"
			"\n"
			"You can try to run Universe Viewer, but it will probably crash or run\n"
