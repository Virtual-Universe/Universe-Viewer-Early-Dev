...
bool LLImageDimensionsInfo::getImageDimensionsJp2()
{
	// Make sure the file is long enough.
	const S32 DATA_LEN = 16; // SOC (2) SIZ (2) Lsiz (2) Rsiz (2) Xsiz(4) Ysiz(4)
	if (!checkFileLength(DATA_LEN))
	{
		llwarns << "Premature end of file" << llendl;
		return false;
	...
	// Read JP2 signature.
	const size_t JP2_MAGIC_SIZE = 4;
	const U8 jp2_magic[JP2_MAGIC_SIZE] = { 0xFF, 0x4F, 0xFF, 0x51 };
	U8 signature[JP2_MAGIC_SIZE];
	mInfile.read((void*)signature, JP2_MAGIC_SIZE);

	if (memcmp(signature, jp2_magic, JP2_MAGIC_SIZE) != 0)
	{
		llwarns << "Not a JP2" << llendl;
		return false;
	}

	// Read image dimensions.
	mInfile.seek(APR_CUR, 4);
	mWidth = read_s32();
	mHeight = read_s32();
