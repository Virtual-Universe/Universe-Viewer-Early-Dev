...
//				std::cerr << "Setting dirty, " << mWidth << " x " << mHeight << std::endl;
	...
	bool initBrowserWindow()
	{
		...
		if(!mNoFollowScheme.empty())
		{
			LLQtWebKit::getInstance()->setNoFollowScheme( mBrowserWindowId, mNoFollowScheme);
		}
...
//				std::cerr << "MediaPluginWebKit::receiveMessage: unknown base message: " << message_name << std::endl;
		...
		else if(message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
		{
			...
			else if(message_name == "set_no_follow_scheme")
			{
				mNoFollowScheme = message_in.getValue("scheme");
				if ( mInitState > INIT_STATE_INITIALIZED )
				{
					LLQtWebKit::getInstance()->setNoFollowScheme( mBrowserWindowId, mNoFollowScheme);
				}
