...
void handle_export_av_texture(void*);
BOOL enable_export_av_texture(void*);
...
class LLAVExportTexture : public view_listener_t
{
	bool handleEvent(const LLSD& userdata)
	{
		std::string texture_type = userdata.asString();
		if ("skin" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_SKIN );
		}
		else if ("hair" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_HAIR );
		}
		else if ("eyes" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_EYES );
		}
		else if ("shirt" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_SHIRT );
		}
		else if ("pants" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_PANTS );
		}
		else if ("shoes" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_SHOES );
		}
		else if ("socks" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_SOCKS );
		}
		else if ("jacket" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_JACKET );
		}
		else if ("gloves" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_GLOVES );
		}
		else if ("undershirt" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_UNDERSHIRT );
		}
		else if ("underpants" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_UNDERPANTS );
		}
		else if ("skirt" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_SKIRT );
		}
		else if ("alpha" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_ALPHA );
		}
		else if ("tattoo" == texture_type)
		{
			handle_export_av_texture( (void*)LLWearableType::WT_TATTOO );
		}
...
class LLAVEnableExportTexture : public view_listener_t
{
	bool handleEvent(const LLSD& userdata)
	{
		std::string texture_type = userdata.asString();
		...
		if ("skin" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_SKIN );
		}
		else if ("hair" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_HAIR );
		}
		else if ("eyes" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_EYES );
		}
		else if ("shirt" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_SHIRT );
		}
		else if ("pants" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_PANTS );
		}
		else if ("shoes" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_SHOES );
		}
		else if ("socks" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_SOCKS );
		}
		else if ("jacket" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_JACKET );
		}
		else if ("gloves" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_GLOVES );
		}
		else if ("undershirt" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_UNDERSHIRT );
		}
		else if ("underpants" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_UNDERPANTS );
		}
		else if ("skirt" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_SKIRT );
		}
		else if ("alpha" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_ALPHA );
		}
		else if ("tattoo" == texture_type)
		{
			newvalue = enable_export_av_texture( (void*)LLWearableType::WT_TATTOO );
		}
...
//void handle_continue_export_texture(AIFilePicker* filepicker, EWearableType index)
void handle_continue_export_texture(std::string folder, LLWearableType::EType index)
{
	...
	LLUUID asset_id;
	std::string name;
	JCExportTracker *objBackup = JCExportTracker::getInstance();

	//if (!filepicker->hasFilename()) return;
	//std::string folder = gDirUtilp->getDirName(filepicker->getFilename());

	switch(index) {
		case LLWearableType::WT_SKIN:
			asset_id = gAgentAvatarp->getTEImage(TEX_HEAD_BODYPAINT)->getID();
			name = "Skin Head";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_BODYPAINT)->getID();
			name = "Skin Upper";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_BODYPAINT)->getID();
			name = "Skin Lower";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_HAIR:
			asset_id = gAgentAvatarp->getTEImage(TEX_HAIR)->getID();
			name = "Hair";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_EYES:
			asset_id = gAgentAvatarp->getTEImage(TEX_EYES_IRIS)->getID();
			name = "Eyes";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_SHIRT:
			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_SHIRT)->getID();
			name = "Shirt";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_PANTS:
			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_PANTS)->getID();
			name = "Pants";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_SHOES:
			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_SHOES)->getID();
			name = "Shoes";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_SOCKS:
			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_SOCKS)->getID();
			name = "Socks";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_JACKET:
			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_JACKET)->getID();
			name = "Jacket Upper";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_JACKET)->getID();
			name = "Jacket Lower";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_GLOVES:
			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_GLOVES)->getID();
			name = "Gloves";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_UNDERSHIRT:
			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_UNDERSHIRT)->getID();
			name = "Undershirt";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_UNDERPANTS:
			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_UNDERPANTS)->getID();
			name = "Underpants";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_SKIRT:
			asset_id = gAgentAvatarp->getTEImage(TEX_SKIRT)->getID();
			name = "Skirt";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_ALPHA:
			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_ALPHA)->getID();
			name = "Alpha Upper";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_ALPHA)->getID();
			name = "Alpha Lower";
			objBackup->exportTexture(asset_id, folder, name);
			break;

		case LLWearableType::WT_TATTOO:
			asset_id = gAgentAvatarp->getTEImage(TEX_HEAD_TATTOO)->getID();
			name = "Tattoo Head";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_UPPER_TATTOO)->getID();
			name = "Tattoo Upper";
			objBackup->exportTexture(asset_id, folder, name);

			asset_id = gAgentAvatarp->getTEImage(TEX_LOWER_TATTOO)->getID();
			name = "Tattoo Lower";
			objBackup->exportTexture(asset_id, folder, name);
		case LLWearableType::WT_SHAPE:
			llwarns << "Shape type unhandled" << llendl;
			break;
		case LLWearableType::WT_PHYSICS:
			llwarns << "Physics type unhandled" << llendl;
			break;
		case LLWearableType::WT_COUNT:
		case LLWearableType::WT_INVALID:
		case LLWearableType::WT_NONE:
			llwarns << "Unhandled type" << llendl;
			break;
	}
}

void handle_export_av_texture(void* data)
{
	LLWearableType::EType index = (LLWearableType::EType)((intptr_t)data);

	std::string proposed_name;
	LLDirPicker& picker = LLDirPicker::instance();
	if(!picker.getDir(&proposed_name)) {
		llwarns << "EXPORT cancelled" << llendl;
		return; //Canceled!
	}
	std::string dir_name = picker.getDirName();
	if(dir_name.empty()) {
		llwarns << "EXPORT bad dir" << llendl;
		return; //Bad dir
	}

	handle_continue_export_texture(dir_name, index);

	//for singularity and all
	//AIFilePicker* filepicker = AIFilePicker::create();
	//filepicker->open("default", FFSAVE_ALL);
	//filepicker->run(boost::bind(&handle_continue_export_texture, filepicker, index));
}

BOOL enable_export_av_texture(void* data)
{
	LLWearableType::EType index = (LLWearableType::EType)((intptr_t)data);
	return gAgentWearables.selfHasWearable(index);
}
...
// [/RLVa:KB]
	...
	//export
	view_listener_t::addMenu(new LLAVExportTexture(), "Export.AVExportTexture");
	view_listener_t::addMenu(new LLAVEnableExportTexture(), "Export.AVEnableExportTexture");
