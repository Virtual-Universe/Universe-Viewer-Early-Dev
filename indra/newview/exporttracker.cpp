...
//adaption for firestorm (v3 type viewer) both gui and code, also using jp2 file cache for export iso encoding/decoding to jp2 decreases image losses - nhede (04-26-2012)
...
void ExportTrackerFloater::buildExportListFromSelection()
{
		...
		iter != mSelection->valid_root_end(); iter++)
	{
		...
		if(object)
		{
			...
			if(object->getRegion()->getHandle() != gAgent.getRegion()->getHandle())
			{
				JCExportTracker::getInstance()->error("", object->getLocalID(), object->getPosition(), "Not in same region as avatar");
				break;
			}
...
LLSD * JCExportTracker::subserialize(LLViewerObject* linkset)
{
	...
	while ((object_index < export_objects.count()))
	{
		...
		// Flags
		prim_llsd["shadows"] = TRUE; // object->flagCastShadows();
		prim_llsd["phantom"] = object->flagPhantom();
		prim_llsd["physical"] = (BOOL)(object->flagUsePhysics());
...
void JCExportTracker::doexportworker(void *userdata)
{
	...
	while(!still_going.empty())
	{
		...
		//LK temp disable this //requested_inventory.push_back(req);
	}
